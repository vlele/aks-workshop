...
az aks show -g $RESOURCE_GROUP_NAME -n $CLUSTER_NAME --query nodeResourceGroup

MC_RESOURCE_GROUP_NAME="<Paste-Resource-Group-Name-From-Above-Command-Output>"
...
# And we can now write into one pod and read from other pod to see that the both mount the same file system
